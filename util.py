...
def update_db(filename, j_list):
	...
	# Iterate through all of todays stories
	# Iterate through all of the stored stories
	# Only add new stories

	# for each of todays stories
	for j in j_list:
		exists = 0
		# for each of the stored stories
		for j2 in db:
			# Flip the switch if there's a match
			if(j['link'].strip() == j2['link'].strip()):
				exists = 1
	
		# If no match detected, append to file
		if(exists == 0):
